...
def mixup_target(target: paddle.Tensor, num_classes, lam=1., smoothing=0.0):
    if target.ndim == 1:
        ...
        output = output * lam + paddle.flip(output,0) * (1. - lam)
    return output
...
class Mixup:
    ...
    def _mix_elem(self, x):
        ...
        return paddle.unsqueeze(paddle.tensor(lam_batch, dtype=x.dtype),axis=1)
...
class FastCollateMixup(Mixup):
    ...
    def _mix_elem_collate(self, output, batch, half=False):
        ...
        return paddle.unsqueeze(paddle.to_tensor(lam_batch),axis=1)
